...
const Mutations = {
    ...
    async requestReset(parent, args, ctx, info) {
        ...
        if(!user) {
            throw new Error(`No user with the email ${args.email} found.`)
        }
        // 2. Set a reset tokjen and expiry on that user
    ...
    async resetPassword(parent, args, ctx, info) {
        ...
        const password = await bcrypt.hash(args.password, 10)
        // 5. save the new password to the user and remove old resetTokenfields
    ...
    async createOrder(parent, args, ctx, info) {
        ...
        // 2. Recalculate the totalfor the price.
